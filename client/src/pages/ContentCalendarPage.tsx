...
export default function ContentCalendarPage() {
  ...
  const run = trpc.contentCalendar.run.useMutation({
    onSuccess: (data) => setResult(data as ContentResult),
  });
  ...
  return (
    <div className="p-6 space-y-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium">Content Goal *</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={contentGoal}
            onChange={(e) => setContentGoal(e.target.value)}
            placeholder="e.g. Increase engagement by 30% over 60 days"
            required
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="e.g. SMB founders, marketing managers"
          />
        </div>
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium">Content Pillars</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={contentPillars}
            onChange={(e) => setContentPillars(e.target.value)}
            placeholder="e.g. Education, Social Proof, Product, Culture"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="e.g. 5x/week LinkedIn, 3x/week Instagram, 1x/week Email"
          />
        </div>

        <div className="space-y-1">
          <label className="text-xs font-medium">Campaign Context</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={campaignContext}
            onChange={(e) => setCampaignContext(e.target.value)}
            placeholder="e.g. Q2 product launch, seasonal promo, always-on"
          />
        </div>

        <div className="space-y-1">
          <label className="text-xs font-medium">Offers</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={offers}
            onChange={(e) => setOffers(e.target.value)}
            placeholder="e.g. Free trial, webinar, ebook download"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <textarea
            ...
            placeholder="Any constraints, tone, brand rules, or context..."
          />
        </div>

        <button
          type="submit"
          disabled={run.isPending || !brandName.trim() || !contentGoal.trim()}
          className="px-4 py-2 text-sm font-medium bg-primary text-primary-foreground rounded disabled:opacity-50"
        >
          {run.isPending ? "Generating Calendar…" : "Generate Content Calendar"}
        </button>
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <h2 className="text-sm font-semibold">Content Calendar Ready</h2>
            {result.taskId && (
              <Link
