...
export default function StrategyFlowPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    run.mutate({
      ...
      currentOffers:  currentOffers.trim()  || undefined,
  ...
  return (
    <div className="p-6 space-y-8 max-w-3xl">
      <h1 className="text-lg font-semibold">Strategy Generation</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium">Business Goal *</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={businessGoal}
            onChange={(e) => setBusinessGoal(e.target.value)}
            placeholder="e.g. Grow MRR by 40% in 6 months"
            required
          />
        </div>
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium">Current Offers</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={currentOffers}
            onChange={(e) => setCurrentOffers(e.target.value)}
            placeholder="e.g. SaaS subscription, consulting, training"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="e.g. LinkedIn, Email, SEO, Paid"
          />
        </div>
        ...
        <button
          type="submit"
          disabled={run.isPending || !brandName.trim() || !businessGoal.trim()}
          className="px-4 py-2 text-sm font-medium bg-primary text-primary-foreground rounded disabled:opacity-50"
        >
          {run.isPending ? "Generating Strategy…" : "Generate Strategy"}
        </button>
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <h2 className="text-sm font-semibold">Strategy Complete</h2>
            {result.taskId && (
              <Link
