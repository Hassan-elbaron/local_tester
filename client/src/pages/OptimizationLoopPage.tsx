...
export default function OptimizationLoopPage() {
  ...
  const run = trpc.optimizationLoop.run.useMutation({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    run.mutate({
      ...
      currentPerformance: currentPerformance.trim() || undefined,
      bottlenecks:        bottlenecks.trim()        || undefined,
  ...
  return (
    <div className="p-6 space-y-8 max-w-3xl">
      <h1 className="text-lg font-semibold">Optimization Loop</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium">Optimization Goal *</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={optimizationGoal}
            onChange={(e) => setOptimizationGoal(e.target.value)}
            placeholder="e.g. Improve landing page CVR from 2% to 5%"
            required
          />
        </div>

        <div className="space-y-1">
          <label className="text-xs font-medium">Current Performance</label>
          <textarea
            ...
            placeholder="e.g. CVR 1.8%, CPC $4.20, ROAS 1.9x, 320 leads/month"
          />
        </div>

        <div className="space-y-1">
          <label className="text-xs font-medium">Bottlenecks</label>
          <input
            className="w-full border rounded px-3 py-2 text-sm bg-background"
            value={bottlenecks}
            onChange={(e) => setBottlenecks(e.target.value)}
            placeholder="e.g. High bounce on LP, low email open rate, weak CTA"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="e.g. Google Ads, LinkedIn, Email, SEO"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="e.g. SMB founders, warm retargeting list"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="e.g. Free trial, demo booking, lead magnet"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="e.g. https://example.com/trial"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <textarea
            ...
            placeholder="Any constraints, previous tests, or context..."
          />
        </div>

        <button
          type="submit"
          disabled={run.isPending || !brandName.trim() || !optimizationGoal.trim()}
          className="px-4 py-2 text-sm font-medium bg-primary text-primary-foreground rounded disabled:opacity-50"
        >
          {run.isPending ? "Running Optimization…" : "Run Optimization Loop"}
        </button>
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <h2 className="text-sm font-semibold">Optimization Plan Ready</h2>
            {result.taskId && (
              <Link
